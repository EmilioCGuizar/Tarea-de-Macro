...
### (a) 
\begin{enumerate}
\item[] \textit{Obtenga del INEGI una serie anual de los salarios (en términos reales) en México, 
...
### (b)
\begin{enumerate}
\item[] \textit{Obtenga del INEGI una serie anual del empleo total en México, calcule la
...
\begin{enumerate}
\item[] \textit{ Obtenga del INEGI una serie anual del producto interno bruto en términos reales, calcule su tasa de cambio anual, calcule su volatilidad. (Serie 3)}
\end{enumerate}
\textbf{Solucion: }
...
## Ejercicio 3
\textbf{3. Contraste un modelo trivial de la determinación del salario con los datos siguiendo estos pasos:}
...
La Matriz Hussmanns es un esquema integrador que implica un algoritmo, en el cual el enfoque de unidad económica aplica a las modalidades de trabajo independiente (i.e. empleadores y trabajadores por cuenta propia), mientras que el enfoque laboral aplica a modalidades de trabajo dependiente, como es el trabajo asalariado.
...
### (f)  Enuncie algunas conclusiones tentativas sobre los resultados que obtuvo, relacionándolas con los modelos discutidos en clase.
...
```{r, fig.cap="Tasa de Desempleo, T4 2019"}
...
```{r fig.cap="Tasa de Subocupación, T4 2019"}
...
```{r fig.cap="Tasa de Disponibilidad, T4 2019"}
...
```{r fig.cap="Búsqueda de otro Empleo, T4 2019"}
